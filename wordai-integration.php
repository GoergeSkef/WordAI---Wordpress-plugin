<?php
/**
 * Plugin Name: wordai-post-integration
 * Description: A plugin to integrate WordAi for post rewriting and avoid AI.
 * Version: 1.0
 * Author: George Iskef
 */
...
function wordai_enqueue_assets() {
    ...
    wp_enqueue_style('wordai-css', plugin_dir_url(__FILE__) . 'assets/css/style.css');
    wp_localize_script('wordai-js', 'wordai_vars', array('nonce' => wp_create_nonce('wordai-nonce'), 'ajax_url' => admin_url('admin-ajax.php')));
}
...
function process_wordai() {
    ...
    $email = 'georgeskef@outlook.com';
    $apiKey = 'a75e0b881ccf7ffd44f1405fb248178c';
    ...
    //set_transient('wordai_debug_content', $post_content, 120); // Store for 60 seconds
    ...
    error_log('WordAi Integration: Sending request to WordAi API with mode ' . $mode);
    error_log(print_r($bodyArgs, true)); // Log the parameters
    
    $response = wp_remote_post($apiUrl . $apiEndpoint, array(
        'body' => $bodyArgs,
        'timeout' => 120 // Increase timeout to 15 seconds
    ));
    
    
    if (is_wp_error($response)) {
        error_log('WordAi Integration: Error in wp_remote_post - ' . $response->get_error_message());
        wp_send_json_error(array('message' => 'Error connecting to WordAi API: ' . $response->get_error_message()));
        return;
    }
    ...
    if ($data['status'] === 'Success') {
        ...
        error_log('WordAi Integration: API error - ' . $errorMessage);
    ...
    */
    ...
    error_log('WordAi Integration: Post content updated successfully for post ID ' . $post_id);
    wp_send_json_success(array('message' => 'Post updated successfully!'));
